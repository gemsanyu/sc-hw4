...
import pathlib
from test_harness_2 import run_harness_2
...
if __name__ == "__main__":
    ...
    checkpoint_dir = pathlib.Path()/"checkpoints"/"harness_2"
    log_dir = pathlib.Path()/"logs"/"full"
    population.add_reporter(EarlyStoppingReport(run_harness_2, checkpoint_dir, fitness_target=10000))
    # population.add_reporter(neat.Checkpointer(generation_interval=10))
    evaluator = ParallelEvaluator(12, run_harness_2)
    # Run NEAT
    try:
